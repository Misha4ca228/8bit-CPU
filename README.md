...
Память: Эмулятор имеет 65536 Байт памяти, 16 последних байт предназначены для вывода символов на экран.

| Диапазон (HEX) | Диапазон (DEC) | Назначение                     |
|----------------|----------------|--------------------------------|
| 0x0000–0xFFDF  | 0–65503        | Основная память                |
| 0xFFE0–0xFFEF  | 65504–65519    | Стек (16 байт)                 |
| 0xFFF0–0xFFFF  | 65520–65535    | Буфер вывода символов на экран |

Также эмулятор имеет 10 регистров, каждый регистр имеет память 1 байт, и PC (Program Counter) для указателя текущей
инструкции и стек который находится в промежутке от 65520–65535

## Терминал

![Терминал](./assets/terminal.png)

Для вывода символов на экран терминала требуется загрузить код символа в соответствующую ячейку память от 0xFFF0–0xFFFF(65520–65535).
...
Все адреса являются 16-битными (диапазон `0–65535`).
...
|  2  | 0b00000010 |  **LD**  | r, addr (16-бит) | `[opcode][r][AH][AL]`     | Загрузить в `r` значение из памяти `addr`            |
|  3  | 0b00000011 |  **ST**  | r, addr (16-бит) | `[opcode][r][AH][AL]`     | Записать `r` в память по адресу `addr`               |
...
| 24  | 0b00011000 | **JMP**  |  addr (16-бит)   | `[opcode][AH][AL]`        | Переход на `addr`                                    |
| 25  | 0b00011001 |  **JZ**  | r, addr (16-бит) | `[opcode][r][AH][AL]`     | Переход, если `r == 0`                               |
| 26  | 0b00011010 | **JNZ**  | r, addr (16-бит) | `[opcode][r][AH][AL]`     | Переход, если `r != 0`                               |
...
- Максимальная длина программы — **65503 байт**.
